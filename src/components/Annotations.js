...
const Annotations = () => {
  ...
  const addSection = (sectionType) => {
    if (notebook.notebook.length === 0) {
      ...
      setNotebook({ notebook: [...notebook.notebook, { sectionType, value: '' }]})
    }
  }
  ...
  return (
    <div className='laboratorioStyle'>
      <div className='editor'>
        ...
        <button className='boton' type="button" onClick={saveNotebook}>Guardar cuaderno</button>
      </div>
      <div className='paper'>
        <div className="estiloCompleto">
          {notebook.notebook.map((contentBlock, index) => (
            <textarea
              ...
              key={(index).toString()}
              className={(contentBlock.sectionType === 'code') ? 'pruebaEmotion' : 'pruebaEmotion2'}
              onChange={(event) => {
